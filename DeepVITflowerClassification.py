...
from deep_VIT import VIT  # 导入Deep_VIT
import numpy as np
import matplotlib.pyplot as plt

plt.rcParams['font.sans-serif'] = ['SimHei']  # 绘图显示中文-
# 参数设置
batch_size = 12  # 每个step处理10张图片
epochs = 80  # 训练轮数
...
#weightpath = './pre_weights/vit_base_patch16_224.pth'
# 训练时保存权重文件的位置
savepath = './flower_weights/Deep_W/'
#savepath = './saved_weights/'
...
for epoch in range(epochs):
    ...
    with torch.no_grad():  # 接下来不计算梯度
        ...
        for (images, labels) in dataloader['val']:
            ...
            pred = logits.argmax(dim=1)  # [b,5]==>[b]
    ...
    #保存权重
        file = open('./flower_weights/Deep_W/12_size_acc.txt', 'a')
        file.write(' ' + str(val_acc.item()))
        file.close()
        file = open('./flower_weights/Deep_W/12_size_tranloss.txt', 'a')
